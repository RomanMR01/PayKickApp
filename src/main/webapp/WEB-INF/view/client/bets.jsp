        ...
        <div class="row">
            <div class="col s12 l8 offset-l2">
                <ul class="tabs transparent">
                    <c:forTokens var="status" items="ALL,ACTIVE,WON,LOST,CANCELED"
                                 delims=",">
                                ...
                                <c:forEach var="singleBet" items="${totalBet.singleBets}">
                                    <tr>
                                        <td><fmt:formatDate type="both" dateStyle="short"
                                                            timeStyle="short" value="${totalBet.date}" /></td>
                                        <td><c:out value="${singleBet.game.title}"></c:out></td>
                                        <td><c:out value="${singleBet.category}"></c:out></td>
                                        <c:choose>
                                            <c:when test="${singleBet.category=='RESULT'}">
                                                <td><c:out value="${singleBet.betResult.result}"></c:out></td>
                                            </c:when>
                                            <c:when test="${singleBet.category=='GOALS'}">
                                                <td><c:out
                                                        value="${singleBet.betTotalGoals.totalGoal}"></c:out></td>
                                            </c:when>
                                            <c:when test="${singleBet.category=='SCORE'}">
                                                <td>${singleBet.betScore.firstTeamScore}-
                                                        ${singleBet.betScore.secondTeamScore}</td>
                                            </c:when>
                                            <c:when test="${singleBet.category=='PLAYER'}">
                                                <td><c:out
                                                        value="${singleBet.betPlayer.player.fulName}"></c:out></td>
                                            </c:when>
                                        </c:choose>

                                        <td><c:out value="${singleBet.status}"></c:out></td>
                                        <td>вЂ”</td>
                                        <td><c:set var="totalCoefficient"
                                                   value="${totalCoefficient*singleBet.coefficient}"></c:set>
                                            <c:out value="${singleBet.coefficient}"></c:out></td>
                                        <td>вЂ”</td>
                                    </tr>
                                </c:forEach>
        ...
        <c:choose>
            ...
            <c:otherwise>
                <li class="material-icons"><a
                        href="bets?type=${type}&page=${page-1}&itemsOnPage=${itemsOnPage}"><i
                        class="material-icons">chevron_left</i></a></li>
            </c:otherwise>
        </c:choose>
            ...
            <c:choose>
                ...
                <c:otherwise>
                    <li class="waves-effect"><a
                            href="bets?type=${type}&page=${i}&itemsOnPage=${itemsOnPage}">
                        <c:out value="${i}"></c:out>
                    </a></li>
                </c:otherwise>
            </c:choose>
